...
uses
  ...
  System.IOUtils,
  System.SysUtils,
  System.Math.Vectors, Winapi.Windows;
...
begin
  ...
  if ALocation = -1 then
    OutputDebugString(PWideChar('Uniform ''' + name + ''' not found.'));
  glUniform1i(ALocation, value);
end;
...
//  AName_: PAnsiChar;
begin
//  AName := RawByteString(name);
//  AName_ := MarshaledAString(AName);
  ALocation := glGetUniformLocation(FID, PAnsiChar(AnsiString(name)));
  if ALocation = -1 then
    OutputDebugString(PWideChar('Uniform ''' + name + ''' not found.'));
